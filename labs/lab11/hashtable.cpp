...
void HashTable::remove(string str) {
	...
	//TODO
	//if the string is in the hash table
	if(find(str)){
		unsigned loc = hash;
		//move through the table starting at the hash to find the string
		while(*data[loc] != str){
			//move to next index
			loc++;
			//ensure hash does not exceed bounds (i.e. wrap around)
			if(loc == size){
				loc = 0;
			}
		}
		//set the data index where the string is to NULL
		data[loc] = NULL;
		//move to next index, wrap around if need be
		loc++;
		if(loc == size){
			loc = 0;
		}

		//move through all the subsequent entries with the same hash
		while(data[loc] != NULL && loc != hash){
			//extract the string
			string temp = *data[loc];
			//delete it
			data[loc] = NULL;
			//reinsert it
			insert(temp);
