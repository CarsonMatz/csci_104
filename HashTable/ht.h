...
bool HashTable<K,V,Prober,Hash,KEqual>::empty() const
{
    ...
    for(auto it : table_){
        //if an occupied slot is reached
        if(it != nullptr){
            //if that occupied slot was not deleted
            if(it->deleted == false){
                //then table not empty
                return false;
            }
        }
    }
    //otherwise all locations were empty or deleted
    return true;
}
...
size_t HashTable<K,V,Prober,Hash,KEqual>::size() const
{
    ...
    for(auto it : table_){
        if(it != nullptr){
            if(it->deleted == false){
                //add to count for every location that is filled by a non-deleted item
                cnt++;
            }
        }
    }
...
{
    //if the size of the table (# of entries) is greater than the percentage of the table wanted, resize it
    //size_t s = size();
    if(totalItems_ >= resizeAlpha_*CAPACITIES[mIndex_]){
        resize();
    }
    //create an index variable and probe to find its location
    ...
    if(i == npos){
        ...
        //check to see if the item already in table, updated the value and make sure not marked as deleted
        if(table_[i] != nullptr){
            table_[i]->item = p;
            table_[i]->deleted = false;
        }
        //otherwise need to create new item
        else{
...
{
    //find the hash item from the key ans et the deleted object to true
...
{
    //move on to the next index in the capacities vector, throw an exception if there isn't another
    mIndex_++;
    if(mIndex_ >= 28){ //28 is size of capacities array
        throw std::logic_error("Out of space");
    }
        //create a copy of the current table
    std::vector<HashItem*> temp = table_;
    //reset the hash table to all null for the new size
    table_.assign(CAPACITIES[mIndex_], nullptr);
    //iterate through the old hash table
    for(auto it : temp){
        //for all entries that are not null
        if(it != nullptr){
            //insert all non-deleted items into list
            if(it->deleted == false){
                //insert(it->item);
                HASH_INDEX_T i = probe(it->item.first);
                table_[i] = new HashItem(it->item);
            }
            else{
                totalItems_--;
            }
